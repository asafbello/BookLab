...
const REVIEW_URL = 'http://localhost:3003/review/:id/:collection';
...
function updateBookAndUser(objToUpdateBook,objToUpdateUser,userId,bookId){
    ...
    var prm1 = axios.put(`${REVIEW_URL}/${bookId}`, objToUpdateBook)
        .then(res =>res.data)
        .catch(err => console.log(err))

    var prm2 = axios.put(`${REVIEW_URL}/${userId}`,objToUpdateUser)
        .then(res =>res.data)
        .catch(err => console.log(err))
        return Promise.all([prm1,prm2])
                .then(fullRes => console.log(fullRes))
